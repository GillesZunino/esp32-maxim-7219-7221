# Sample: Advanced Cascading — Per-Device Mode Control

This sample demonstrates how to independently control the operating mode of individual MAX7219 / MAX7221 devices in a three-device cascade. Each device is loaded with its own symbol pattern and then cycled between normal, shutdown, and test modes independently of the others.
...
3. Configure scan limit to eight digits on all devices with `led_driver_max7219_configure_chain_scan_limit()`,
4. Configure Code-B decoding on all devices with `led_driver_max7219_configure_chain_decode()`,
5. Write digit patterns to individual devices using `led_driver_max7219_set_digits()`,
6. Set and cycle the operating mode (normal, shutdown, test) of individual devices using `led_driver_max7219_set_mode()`,
...
## Firmware
In `max7219_7221_advanced_cascade.c`, configure `CS_LOAD_PIN` (`/CS`), `CLK_PIN` (`CLK`) and `DIN_PIN` (`MOSI`) adequately for your hardware setup:
```c
