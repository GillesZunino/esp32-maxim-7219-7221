...
void app_main(void) {
    ...
    maxim7219_intensity_t intensity = MAXIM7219_INTENSITY_DUTY_CYCLE_STEP_1;
    do {
        // Set intensity on all devices - MAXIM7219_INTENSITY_DUTY_CYCLE_STEP_1 is dimest
        ESP_LOGI(TAG, "Set intensity to '%d' on all devices in the chain", intensity);
        ESP_ERROR_CHECK(led_driver_max7219_set_chain_intensity(led_maxim7219_handle, intensity));
        intensity = intensity + 1;
        if (intensity > MAXIM7219_INTENSITY_DUTY_CYCLE_STEP_16) {
            intensity = MAXIM7219_INTENSITY_DUTY_CYCLE_STEP_1;
        }
